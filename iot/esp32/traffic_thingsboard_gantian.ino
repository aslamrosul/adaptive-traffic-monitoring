 ...
 * - Traffic light automation bergantian
...
void setup()
{
  ...
  Serial.println("Traffic Light: Alternating Mode");
  Serial.println("=======================================");
...
int calculateDensityLevel(bool irDetected, int distance)
{
  bool ultrasonicDetected =
    distance > 0 && distance <= ULTRASONIC_THRESHOLD;
...
void cycleTrafficLights()
{
  ...
  // Jalankan hanya saat phase berubah
  if (phase != lastPhase)
  {
    if (phase == 0)
    {
