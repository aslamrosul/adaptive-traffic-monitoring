 ...
 *
 * Sensor:
 * - IR obstacle sensor untuk membantu mendeteksi kendaraan
 * - HC-SR04 untuk menghitung kendaraan dan membantu menentukan tingkat kepadatan
...
const char *ssid = "PIXMA MP287";
const char *wifiPassword = "Seafood99";

// ============================================================
// MQTT CONFIG
// ============================================================

#define MQTT_SERVER "13.54.179.237" // IP baru astraea.my.id (ganti astraea.my.id jika DNS)
...
String topicYolo()
{
  return "traffic/" + String(DEVICE_ID) + "/yolo";
}

String topicCamYoloData()
{
  return "traffic/CAM_YOLO_01/data";
}
...
// HC-SR04 raw previous state untuk debug vehicle counting
bool lastNorthCountDetected = false;
bool lastSouthCountDetected = false;
bool lastEastCountDetected = false;

// HC-SR04 latch/occupied state untuk mencegah dobel count
// selama mobil masih berhenti lama di depan sensor.
bool northCountOccupied = false;
bool southCountOccupied = false;
bool eastCountOccupied = false;

unsigned long northCountClearStartedAt = 0;
unsigned long southCountClearStartedAt = 0;
unsigned long eastCountClearStartedAt = 0;

// Debounce agar satu kendaraan tidak terhitung berkali-kali saat masuk area sensor.
unsigned long lastNorthCountAt = 0;
unsigned long lastSouthCountAt = 0;
unsigned long lastEastCountAt = 0;

const unsigned long VEHICLE_COUNT_DEBOUNCE_MS = 200;

// Mobil baru boleh dihitung lagi kalau area HC-SR04 sudah benar-benar kosong
// secara stabil selama durasi ini. Ini mencegah bug: mobil diam lama,
// bacaan HC-SR04 sempat 0/noise, lalu dihitung sebagai mobil baru.
const unsigned long ULTRASONIC_CLEAR_HOLD_MS = 1000;
...
// Jika true: vehicle count & density diambil dari YOLO AWS (CAM hanya rekam), bukan IR/HC-SR04
bool useYoloMode = false; // sensor mode
...
bool isUltrasonicDetected(int distanceCm);
...
bool updateVehicleCount(
    int northCountDistanceCm,
    int southCountDistanceCm,
    int eastCountDistanceCm);

bool updateLaneVehicleCount(
    const char *laneName,
    int distanceCm,
    bool &occupied,
...
void setup()
{
  ...
  Serial.println("HC-SR04 COUNT + IR + PHYSICAL LED");
  Serial.println("=======================================");
...
void subscribeAllTopics()
{
  ...
  client.subscribe(topicYolo().c_str());
  client.subscribe(topicCamYoloData().c_str());

  Serial.println("Subscribed device-specific MQTT topics (incl. YOLO + CAM_YOLO_01).");
}
...
void readRealSensorData()
{
  ...
  bool northCountDetected = isUltrasonicDetected(northDistanceCm);
  ...
  Serial.print(" | Count HC: ");
  Serial.print(northCountDetected ? "DETECTED" : "CLEAR");
  ...
  Serial.print(" | Count HC: ");
  Serial.print(southCountDetected ? "DETECTED" : "CLEAR");
  Serial.print(" | Distance: ");
  ...
  Serial.print(" | Count HC: ");
  Serial.print(eastCountDetected ? "DETECTED" : "CLEAR");
  Serial.print(" | Distance: ");
    ...
    int eastCountDistanceCm)
{
  ...
  countChanged |= updateLaneVehicleCount(
      "NORTH",
      northCountDistanceCm,
      northCountOccupied,
      northCountClearStartedAt,
      lastNorthCountDetected,
      lastNorthCountAt,
      northCount);

  countChanged |= updateLaneVehicleCount(
      "SOUTH",
      southCountDistanceCm,
      southCountOccupied,
      southCountClearStartedAt,
      lastSouthCountDetected,
      lastSouthCountAt,
      southCount);
  ...
  countChanged |= updateLaneVehicleCount(
      ...
      eastCountDistanceCm,
      eastCountOccupied,
      eastCountClearStartedAt,
      lastEastCountDetected,
      lastEastCountAt,
      eastCount);
...
{
  unsigned long now = millis();
  bool rawDetected = isUltrasonicDetected(distanceCm);
  bool countChanged = false;

  /*
   * Count hanya saat objek pertama kali masuk area HC-SR04
   * dan state sebelumnya benar-benar kosong.
   */
  if (rawDetected)
  {
    clearStartedAt = 0;
    ...
    if (!occupied &&
        now - lastCountAt >= VEHICLE_COUNT_DEBOUNCE_MS)
    {
      vehicleCount++;
      lastCountAt = now;
      countChanged = true;

      Serial.print("New vehicle counted by HC-SR04: ");
  ...
  {
    /*
     * Jangan langsung reset occupied ketika HC-SR04 baca 0 / miss.
     * Mobil yang diam kadang membuat bacaan ultrasonic berkedip:
     * 4 cm -> 0 -> 4 cm. Kalau langsung reset, counter jadi dobel.
     */
    if (occupied)
    {
      if (clearStartedAt == 0)
      ...
      if (now - clearStartedAt >= ULTRASONIC_CLEAR_HOLD_MS)
      {
        occupied = false;
        clearStartedAt = 0;

        Serial.print("HC-SR04 count area clear: ");
        Serial.println(laneName);
      }
    }
  }
    ...
    unsigned int length)
{
  ...
  if (topicString == topicYolo() || topicString == topicCamYoloData())
  {
    ...
    StaticJsonDocument<768> yoloDoc;
    DeserializationError yoloErr = deserializeJson(yoloDoc, rawMessage);
    if (!yoloErr) {
      if (yoloDoc.containsKey("totalVehicles") || yoloDoc["stats"].containsKey("totalVehicles")) {
      ...
      // Format CAM_YOLO_01/data: pakai north/south/east
      if (yoloDoc.containsKey("north_vehicle_count")) {
        northCount = yoloDoc["north_vehicle_count"] | northCount;
        southCount = yoloDoc["south_vehicle_count"] | southCount;
        eastCount = yoloDoc["east_vehicle_count"] | eastCount;
        northDensityLevel = yoloDoc["north_density_level"] | northDensityLevel;
        ...
        Serial.print(" E="); Serial.println(eastCount);
        markTelemetryDirty();
        return;
      }
    }
    Serial.print("YOLO parse gagal: ");
