...
// GET /api/cameras/[cameraId]/snapshot — proxy server-side ke snapshot Server 2.
  ...
  try {
    ...
    try {
      ...
      const upstream = await fetch(
        `${baseUrl}/v1/cameras/${encodeURIComponent(cameraId)}/snapshot.jpg`,
        { headers, signal: ctrl.signal, cache: "no-store" }
      );
      if (!upstream.ok || !upstream.body) {
        return NextResponse.json(
          { success: false, error: "Snapshot tidak tersedia" },
          { status: 502 }
        );
      }
